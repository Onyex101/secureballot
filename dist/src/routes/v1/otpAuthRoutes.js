...
const rateLimitMiddleware = (name, options) => {
    return (0, rateLimiter_1.createRateLimiter)(options.duration * 1000, // Convert seconds to milliseconds
    options.points, `TOO_MANY_${name.toUpperCase()}_REQUESTS`, `Too many ${name} requests from this IP, please try again later.`);
};
 ...
 */
router.post('/voter/request-login', rateLimitMiddleware('voter-login', { points: 5, duration: 300 }), // 5 requests per 5 minutes
(0, middleware_1.validate)([
 ...
 *         description: Server error
 */
router.post('/voter/verify-otp', rateLimitMiddleware('otp-verify', { points: 10, duration: 300 }), // 10 requests per 5 minutes
(0, middleware_1.validate)([
 ...
 *         description: Server error
 */
router.post('/voter/resend-otp', rateLimitMiddleware('otp-resend', { points: 3, duration: 300 }), // 3 requests per 5 minutes
(0, middleware_1.validate)([(0, express_validator_1.body)('userId').isUUID().withMessage('User ID must be a valid UUID')]), otpAuthController.resendOtp);
/**
 * @swagger
 ...
 *         description: Server error
 */
router.post('/admin/login', rateLimitMiddleware('admin-login', { points: 5, duration: 900 }), // 5 requests per 15 minutes
(0, middleware_1.validate)([
    (0, express_validator_1.body)('nin')
        .isLength({ min: 11, max: 11 })
        .withMessage('NIN must be exactly 11 digits')
